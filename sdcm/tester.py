...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def _init_test_timeout_thread(self) -> threading.Timer:
        ...
        end_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(self.start_time + int(self._duration) * 60))
        self.log.info(f'Test start time {start_time}, duration is {self._duration} and timeout set to {end_time}')
