...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def _get_target_nodes(
        ...
        output = [node for node in self.cluster.nodes if not node.running_nemesis]
        if is_seed is not None:
            output = [node for node in output if node.is_seed == is_seed]
        if dc_idx is not None:
            output = [node for node in output if node.dc_idx == dc_idx]
        if rack is not None:
            output = [node for node in output if node.rack == rack]
        return output
    ...
    def disrupt_grow_shrink_cluster(self):
        ...
        if self._is_it_on_kubernetes():
            ...
            InfoEvent(message=f'GrowCluster - Add New node to {rack} rack')
        else:
            rack = 0
